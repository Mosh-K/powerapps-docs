---
title: "Query data using QueryExpression"
description: Learn how to query data using QueryExpression to compose dynamic queries in Microsoft Dataverse. Build complex queries without string manipulation for better performance.
ms.date: 03/26/2026
ms.reviewer: jdaly
ms.topic: how-to
author: MsSQLGirl
...
The [QueryExpression class](xref:Microsoft.Xrm.Sdk.Query.QueryExpression), together with other classes in the [Microsoft.Xrm.Sdk.Query namespace](xref:Microsoft.Xrm.Sdk.Query), provides an object model to compose complex queries to retrieve records from Dataverse by using the [IOrganizationService.RetrieveMultiple method](xref:Microsoft.Xrm.Sdk.IOrganizationService.RetrieveMultiple%2A). [Compare options when querying data using the SDK for .NET](../entity-operations-query-data.md)


## Compose a query

Use `QueryExpression` to compose dynamic queries that you can modify without the string or XML manipulation required [when using FetchXml](../../fetchxml/overview.md).
...
- Specify the table as the [QueryExpression.EntityName property](xref:Microsoft.Xrm.Sdk.Query.QueryExpression.EntityName) by using the [QueryExpression(String) constructor](/dotnet/api/microsoft.xrm.sdk.query.queryexpression.-ctor#microsoft-xrm-sdk-query-queryexpression-ctor(system-string)).
...
You can compose the same query without the [QueryExpression(String) constructor](/dotnet/api/microsoft.xrm.sdk.query.queryexpression.-ctor#microsoft-xrm-sdk-query-queryexpression-ctor(system-string)) or object initialization style. Just set the properties on the instantiated instance as shown in the following example:
...
- Specify the table as the [QueryExpression.EntityName property](xref:Microsoft.Xrm.Sdk.Query.QueryExpression.EntityName) directly after initializing the `QueryExpression` instance by using the default constructor.
...
To limit the number of rows returned, use the [QueryExpression.TopCount property](xref:Microsoft.Xrm.Sdk.Query.QueryExpression.TopCount). If you don't set the `TopCount` property, Dataverse returns up to 5,000 rows for standard tables and 500 for elastic tables.

Alternatively, specify a number of records to return by using paging. Don't use the `TopCount` property when you request pages of data. [Learn how to request paged results](page-results.md).

You can't use the `TopCount` property when you request a count of rows by using the [PagingInfo.ReturnTotalRecordCount property](xref:Microsoft.Xrm.Sdk.Query.PagingInfo.ReturnTotalRecordCount). [Learn to count rows](count-rows.md).
...
As explained in [Query data using the SDK for .NET](../entity-operations-query-data.md), `QueryExpression` is one of three types derived from the [QueryBase class](xref:Microsoft.Xrm.Sdk.Query.QueryBase). You can pass it to the [IOrganizationService.RetrieveMultiple Method](xref:Microsoft.Xrm.Sdk.IOrganizationService.RetrieveMultiple%2A) to get an <xref:Microsoft.Xrm.Sdk.EntityCollection> containing the results.
...
You can also use the [RetrieveMultipleRequest class](xref:Microsoft.Xrm.Sdk.Messages.RetrieveMultipleRequest) to set the query to the [RetrieveMultipleRequest.Query property](xref:Microsoft.Xrm.Sdk.Messages.RetrieveMultipleRequest.Query). Then, send the request by using the [IOrganizationService.Execute Method](xref:Microsoft.Xrm.Sdk.IOrganizationService.Execute%2A).
...
- Include the operation as part of a batch by using the [ExecuteMultipleRequest](../execute-multiple-requests.md) or [ExecuteTransactionRequest](../use-executetransaction.md) classes.
...
- Retrieve data by using the Dataverse Web API. Some [Web API operations enable `QueryExpression` parameters](#use-queryexpression-as-a-message-parameter), but you can't compose a query by using `QueryExpression` to retrieve data through the Web API.
- [Aggregation limitations](aggregate-data.md#queryexpression-aggregation-limitations) lists the following limitations for aggregations by using `QueryExpression`:
...
> [!IMPORTANT]
> If you use the `FetchXmlToQueryExpression` message with either the SDK [FetchXmlToQueryExpressionRequest class](/dotnet/api/microsoft.crm.sdk.messages.fetchxmltoqueryexpressionrequest) or Web API [FetchXmlToQueryExpression function](/power-apps/developer/data-platform/webapi/reference/fetchxmltoqueryexpression), any capabilities that `QueryExpression` doesn't support aren't applied and no error occurs.

## Community tools

The [XrmToolBox](../../community-tools.md#xrmtoolbox) [FetchXMLBuilder](https://fetchxmlbuilder.com/) is a free tool to compose and test FetchXml requests. It also generates code for `QueryExpression` queries by using the same designer experience.
