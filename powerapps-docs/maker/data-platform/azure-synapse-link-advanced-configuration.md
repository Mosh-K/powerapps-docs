...
>
> Not all CUD changes are captured in **append only**: The Synapse Link processes changes in data in groups or "batches" before publishing them to the data lake. As a result, if the user makes changes within a short time interval, not all CUD changes are captured in the data lake.

Here are some more details on when to use either of the options.

- **In place update**:  This option is the default setting and recommended only if you want to connect directly to the data in lake and need the current state (not history or incremental changes). The file contains the full data set and can be utilized via Power BI or by copying the entire dataset for ETL (extract, transform, load) pipelines.
...
When you write Dataverse table data to Azure Data Lake Storage by using Azure Synapse Link, the tables are partitioned in the lake based on the `createdOn` value on each row in the source. The default partition strategy is by month, so data is partitioned in Azure Data Lake on a monthly basis.

Based on the Dataverse table volume and data distribution, you can choose to partition your data by year. With this option, when Dataverse table data is written to the Azure data lake, it's partitioned on a yearly basis based on the `createdOn` value on each row in the source. For tables without the `createdOn` column, the rows of data are partitioned into a new file every 5,000,000 records. This is a per table setting and is available as a checkbox under **Advanced** > **Show advanced configuration settings**.

Here are examples of how data can be handled in the lake with a yearly or monthly partition strategy.
