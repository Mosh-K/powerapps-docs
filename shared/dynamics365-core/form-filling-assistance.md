Copilot provides form fill assistance for apps. This feature suggests AI-generated content for fields, making it easier and faster to enter data in forms. The suggestions are entirely optional and don't get saved until the user explicitly reviews and accepts them.
...
2. Select the **smart paste** icon in the command bar. Alternatively, make sure you don't select any specific fields in the form, and then use the regular paste keyboard shortcut (Ctrl+V or Cmd+V).
...
1. Use the file selector button to select the file you'd like to use to fill in the form. Alternatively, drag and drop the file in the toolbar.
2. Hover over the file's source tag to identify which fields are suggested based on the file's contents.
3. To accept all suggestions from the sources listed in the toolbar, select the accept all button located to the right of the toolbar. This button identifies the number of open suggestions in the form that it accepts.
4. To clear suggestions from the file, select the clear button in the tag. If the toolbar lists multiple sources, clear all suggestions by using the clear all button in the toolbar.
...
:::image type="content" source="/power-apps/user/media/formfill_m365copilotchat.png" alt-text="Screenshot that shows form fill assistance inside Microsoft 365 Copilot chat.":::
